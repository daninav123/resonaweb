...
const CommercialQuoteRequestsManager = ({ apiBasePath = '/quote-requests' }: { apiBasePath?: string }) => {
  ...
  const [formData, setFormData] = useState({
    ...
    // Sistema de montaje por porcentajes
    installationComplexity: 'standard', // 'simple' | 'standard' | 'complex' | 'custom'
  ...
  // Detectar si hay productos que fuerzan montaje complejo
  const hasForceComplexProducts = () => {
    return quoteItems.some(item => {
      const product = products.find(p => p.id === item.id.replace('product-', ''));
      return product?.installationComplexity === 'FORCE_COMPLEX';
    });
  };

  // Calcular coste de montaje según complejidad
  const calculateInstallationCost = () => {
    const subtotal = calculateQuoteTotal();
    
    if (formData.installationComplexity === 'custom') {
      return formData.customInstallationCost || 0;
    }
    
    // Porcentajes según complejidad
    ...
    const percentage = percentages[formData.installationComplexity] || 0.40;
  ...
  const calculateFinalPrice = () => {
    ...
    const rental = formData.rentalCost || 0;
    
    // Si hay precio final manual, usarlo
    if (formData.finalPrice > 0) {
      return formData.finalPrice;
    }
    
    return subtotal + installationCost + transport + rental;
  };
          ...
          <div className="bg-white rounded-lg max-w-6xl w-full max-h-[90vh] overflow-y-auto">
            <div className="p-6">
              ...
              <div className="space-y-6">
                {/* PANEL DE PRECIO FINAL PROMINENTE - ARRIBA DE TODO */}
                <div className="bg-gradient-to-r from-green-500 to-emerald-600 rounded-xl p-6 text-white shadow-lg">
                  <div className="flex items-center justify-between mb-4">
                    <div>
                      <p className="text-sm text-green-100 font-medium mb-1">💰 Precio Final del Presupuesto</p>
                      <p className="text-xs text-green-200">
                        {formData.finalPrice > 0 ? '✏️ Precio personalizado establecido' : '🤖 Calculado automáticamente'}
                      </p>
                    </div>
                  ...
                  <div className="bg-white/10 backdrop-blur rounded-lg p-4">
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                      <div>
                        <p className="text-green-200 text-xs">Productos/Servicios</p>
                        <p className="font-bold text-lg">€{calculateQuoteTotal().toFixed(2)}</p>
                      </div>
                      <div>
                        <p className="text-green-200 text-xs">Montaje ({formData.installationComplexity === 'simple' ? '30%' : formData.installationComplexity === 'standard' ? '40%' : formData.installationComplexity === 'complex' ? '50%' : 'Custom'})</p>
                        <p className="font-bold text-lg">€{calculateInstallationCost().toFixed(2)}</p>
                      </div>
                      {(formData.transportCost || 0) > 0 && (
                        <div>
                          <p className="text-green-200 text-xs">Transporte</p>
                          <p className="font-bold text-lg">€{(formData.transportCost || 0).toFixed(2)}</p>
                        </div>
                      )}
                      {(formData.rentalCost || 0) > 0 && (
                        <div>
                          <p className="text-green-200 text-xs">Alquiler Externo</p>
                          <p className="font-bold text-lg">€{(formData.rentalCost || 0).toFixed(2)}</p>
                        </div>
                      )}
                    </div>
                    
                    {/* Advertencia si hay productos que fuerzan montaje complejo */}
                    {hasForceComplexProducts() && formData.installationComplexity !== 'complex' && formData.installationComplexity !== 'custom' && (
                      <div className="mt-3 bg-yellow-500/20 border border-yellow-400 rounded p-3">
                        <p className="text-sm text-yellow-100 font-medium">
                          ⚠️ Este presupuesto incluye productos que requieren montaje complejo mínimo (50%)
                        </p>
                      </div>
                    )}
                    
                    {/* Campo de precio final manual */}
                    <div className="mt-4 pt-4 border-t border-white/20">
                      <label className="block text-sm font-bold text-white mb-2">
                        ✏️ Establecer Precio Final Personalizado (Opcional)
                      </label>
                      <div className="flex gap-3">
                        <input
                          type="number"
                          ...
                          className="flex-1 px-4 py-3 bg-white text-gray-900 border-2 border-green-300 rounded-lg focus:ring-2 focus:ring-white text-lg font-semibold"
                          placeholder="Dejar vacío para cálculo automático"
                        />
                        {formData.finalPrice > 0 && (
                          <button
                            onClick={() => setFormData({ ...formData, finalPrice: 0 })}
                      ...
                      {formData.finalPrice > 0 && Math.abs(formData.finalPrice - (calculateQuoteTotal() + calculateInstallationCost() + (formData.transportCost || 0) + (formData.rentalCost || 0))) > 0.01 && (
                        <p className="text-xs text-yellow-100 mt-2">
                          💡 Diferencia con precio calculado: €{Math.abs(formData.finalPrice - (calculateQuoteTotal() + calculateInstallationCost() + (formData.transportCost || 0) + (formData.rentalCost || 0))).toFixed(2)}
                        </p>
                      )}
                    </div>
                  </div>
              ...
              <div className="col-span-1 space-y-2">
                ...
                <div className="bg-gradient-to-br from-blue-50 to-indigo-100 border-2 border-blue-500 rounded-xl p-6">
                  <h3 className="text-xl font-bold text-blue-900 mb-4 flex items-center gap-2">
                    <span className="text-2xl">🔧</span>
                    Complejidad del Montaje e Instalación
                  </h3>
                  
                  <p className="text-sm text-blue-700 mb-6">
                    Selecciona la complejidad según el tipo de instalación que requiere el evento
                  </p>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                    ...
                    <button
                      type="button"
                      onClick={() => !hasForceComplexProducts() && setFormData({ ...formData, installationComplexity: 'simple' })}
                      disabled={hasForceComplexProducts()}
                      className={`p-4 rounded-lg border-2 transition-all ${
                        formData.installationComplexity === 'simple'
                          ? 'bg-green-100 border-green-500 shadow-lg transform scale-105'
                          : hasForceComplexProducts()
                    ...
                    >
                      <div className="text-center">
                        ...
                        <p className="font-bold text-lg text-gray-900">Simple</p>
                        <p className="text-sm text-gray-600 mb-2">30% del subtotal</p>
                        <p className="text-2xl font-bold text-green-600">
                          €{(calculateQuoteTotal() * 0.30).toFixed(2)}
                        </p>
                        <p className="text-xs text-gray-500 mt-2">
                          Ground support, totems, configuración básica
                        </p>
                      </div>
                    </button>
                    ...
                    <button
                      type="button"
                      onClick={() => !hasForceComplexProducts() && setFormData({ ...formData, installationComplexity: 'standard' })}
                      disabled={hasForceComplexProducts()}
                      className={`p-4 rounded-lg border-2 transition-all ${
                        formData.installationComplexity === 'standard'
                    ...
                    >
                      <div className="text-center">
                        <div className="text-3xl mb-2">🔨</div>
                        <p className="font-bold text-lg text-gray-900">Estándar</p>
                        <p className="text-sm text-gray-600 mb-2">40% del subtotal</p>
                        <p className="text-2xl font-bold text-blue-600">
                          €{(calculateQuoteTotal() * 0.40).toFixed(2)}
                        </p>
                        <p className="text-xs text-gray-500 mt-2">
                          Estructura volada básica, instalación media
                        </p>
                      </div>
                    </button>
                    ...
                    <button
                      type="button"
                      onClick={() => setFormData({ ...formData, installationComplexity: 'complex' })}
                      className={`p-4 rounded-lg border-2 transition-all ${
                        formData.installationComplexity === 'complex'
                    ...
                    >
                      <div className="text-center">
                        ...
                        <p className="font-bold text-lg text-gray-900">Complejo</p>
                        <p className="text-sm text-gray-600 mb-2">50% del subtotal</p>
                        <p className="text-2xl font-bold text-purple-600">
                          €{(calculateQuoteTotal() * 0.50).toFixed(2)}
                        </p>
                        <p className="text-xs text-gray-500 mt-2">
                          Diseños avanzados, múltiples estructuras, guirnaldas
                        </p>
                      </div>
                    </button>
                    ...
                    <button
                      type="button"
                      onClick={() => setFormData({ ...formData, installationComplexity: 'custom' })}
                      className={`p-4 rounded-lg border-2 transition-all ${
                        formData.installationComplexity === 'custom'
                          ? 'bg-orange-100 border-orange-500 shadow-lg transform scale-105'
                          : 'bg-white border-gray-300 hover:border-orange-400 hover:shadow-md'
                      }`}
                    >
                      <div className="text-center">
                        <div className="text-3xl mb-2">✏️</div>
                        <p className="font-bold text-lg text-gray-900">Personalizado</p>
                        <p className="text-sm text-gray-600 mb-2">Coste manual</p>
                        {formData.installationComplexity === 'custom' ? (
                          <input
                            type="number"
                            min="0"
                            step="0.01"
                            value={formData.customInstallationCost || ''}
                            onChange={(e) => setFormData({ ...formData, customInstallationCost: parseFloat(e.target.value) || 0 })}
                            className="w-full px-3 py-2 border-2 border-orange-400 rounded-lg focus:ring-2 focus:ring-orange-500 font-bold text-center"
                            placeholder="€"
                            onClick={(e) => e.stopPropagation()}
                          />
                        ) : (
                          <p className="text-xl font-bold text-orange-600">€ Custom</p>
                        )}
                        <p className="text-xs text-gray-500 mt-2">
                          Introduce coste manual de montaje
                        </p>
                      </div>
                    </button>
                  </div>
...
export default CommercialQuoteRequestsManager;
