    ...
    return (
      <div className="min-h-screen bg-ink py-8">
        <div className="container mx-auto px-4 max-w-md">
          <div className="bg-ink-800 rounded-lg shadow-md p-10 text-center">
            <Heart className="w-14 h-14 text-cream/70 mx-auto mb-4" />
            <h2 className="text-xl font-semibold text-cream mb-2">Inicia sesión para ver tus favoritos</h2>
            <p className="text-cream/65 mb-6">Guarda los equipos que te interesan y accede a ellos desde cualquier dispositivo.</p>
            <button
  ...
  return (
    <div className="min-h-screen bg-ink py-8">
      <div className="container mx-auto px-4">
        <h1 className="text-3xl font-bold text-cream mb-8">Mis Favoritos</h1>

        {favorites.length === 0 ? (
          <div className="bg-ink-800 rounded-lg shadow-md p-12 text-center max-w-md mx-auto">
            <Heart className="w-16 h-16 text-cream/70 mx-auto mb-4" />
            <h2 className="text-xl font-semibold text-cream mb-2">No tienes favoritos todavía</h2>
            ...
            {favorites.map((fav) => {
              ...
              return (
                <div
                  ...
                  className="bg-ink-800 rounded-lg shadow-md overflow-hidden hover:shadow-lg transition flex flex-col"
                >
                  <Link to={`/productos/${product.slug || productId}`}>
                    <img
                      src={img}
                      alt={product.name}
                      className="w-full h-48 object-contain bg-ink"
                      loading="lazy"
                      onError={(e) => {
                  ...
                  <div className="p-4 flex-1 flex flex-col">
                    <Link to={`/productos/${product.slug || productId}`}>
                      <h3 className="font-semibold text-cream mb-2 line-clamp-2 hover:text-resona-light transition-colors">
                        {product.name}
                      </h3>
                    </Link>
                    <p className="text-resona-light font-bold mb-4">{priceDisplay.main}</p>
                    <div className="flex gap-2 mt-auto">
                      <Link
                        to={`/productos/${product.slug || productId}`}
                        className="flex-1 flex items-center justify-center gap-1.5 px-3 py-2 bg-ink-800 text-cream/75 rounded-lg hover:bg-gray-200 transition text-sm"
                      >
                      ...
                      <button
                        ...
                        className="p-2 border border-cream/10 text-cream/45 rounded-lg hover:bg-red-50 hover:text-red-600 hover:border-red-200 transition"
