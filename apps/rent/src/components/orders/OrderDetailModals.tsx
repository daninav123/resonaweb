...
export const StatusModal = ({ show, newStatus, setNewStatus, onClose, onConfirm, isPending }: StatusModalProps) => {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-ink-800 rounded-lg p-6 max-w-md w-full mx-4">
        <h3 className="text-xl font-bold mb-4">Cambiar Estado del Pedido</h3>
        <div className="mb-4">
          <label className="block text-sm font-medium text-cream/75 mb-2">Nuevo Estado</label>
          <select
            value={newStatus}
            onChange={(e) => setNewStatus(e.target.value)}
            className="w-full px-3 py-2 border border-cream/15 rounded-lg focus:ring-2 focus:ring-blue-500"
          >
        ...
        <div className="flex gap-3">
          <button onClick={onClose} className="flex-1 px-4 py-2 border border-cream/15 rounded-lg hover:bg-white/5 transition">
            Cancelar
          </button>
          <button
            onClick={onConfirm}
            disabled={!newStatus || isPending}
            className="flex-1 px-4 py-2 bg-resona text-white rounded-lg hover:bg-resona-dark transition disabled:opacity-50 disabled:cursor-not-allowed"
          >
...
export const EditModal = ({ show, editData, setEditData, onClose, onSave }: EditModalProps) => {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-ink-800 rounded-lg p-6 max-w-2xl w-full mx-4 max-h-[90vh] overflow-y-auto">
        <h3 className="text-xl font-bold mb-4">Editar Pedido</h3>
        <div className="space-y-4 mb-6">
          <div>
            <label className="block text-sm font-medium text-cream/75 mb-2">Notas del Cliente</label>
            <textarea
              value={editData.notes || ''}
              onChange={(e) => setEditData({ ...editData, notes: e.target.value })}
              rows={3}
              className="w-full px-3 py-2 border border-cream/15 rounded-lg focus:ring-2 focus:ring-indigo-500"
              placeholder="Notas visibles para el cliente..."
            />
          </div>
          <div>
            <label className="block text-sm font-medium text-cream/75 mb-2">Notas Internas (Admin)</label>
            <textarea
        ...
        <div className="flex gap-3">
          <button
            ...
            className="flex-1 px-4 py-2 border border-cream/15 rounded-lg hover:bg-white/5 transition"
          >
            Cancelar
          </button>
      ...
      <div className="bg-ink-800 rounded-lg p-6 max-w-md w-full mx-4">
        ...
        <div className="mb-6">
          ...
          <textarea
            ...
            rows={4}
            required
            className="w-full px-3 py-2 border border-cream/15 rounded-lg focus:ring-2 focus:ring-red-500"
        ...
        <div className="mb-6 space-y-4">
          ...
          <div>
            ...
            <select
              ...
              className="w-full px-3 py-2 border border-cream/15 rounded-lg focus:ring-2 focus:ring-teal-500"
            >
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-ink-800 rounded-lg p-6 max-w-md w-full mx-4">
        ...
        <div className="mb-6 space-y-4">
          <div className="bg-resona/10 border-l-4 border-blue-500 p-3 rounded">
            <p className="text-sm text-resona-light">
              <strong>Importe de la fianza:</strong> {formatPrice(depositAmount)}
            </p>
          </div>
          {depositAction === 'release' && (
            <div>
              <label className="block text-sm font-medium text-cream/75 mb-2">Importe Retenido (opcional)</label>
              <input
                ...
                className="w-full px-3 py-2 border border-cream/15 rounded-lg focus:ring-2 focus:ring-yellow-500"
                placeholder="0.00"
              />
              <p className="text-xs text-cream/50 mt-1">Dejar en 0 para devolver la fianza completa</p>
            </div>
          )}
          <div>
            <label className="block text-sm font-medium text-cream/75 mb-2">Notas (opcional)</label>
            <textarea
              value={depositNotes}
          ...
          {depositAction === 'release' && depositRetainedAmount > 0 && (
            <div className="bg-amber-500/10 border-l-4 border-yellow-500 p-3 rounded">
...
export const QRModal = ({ show, paymentLinkQR, onClose }: QRModalProps) => {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4" onClick={onClose}>
      <div className="bg-ink-800 rounded-lg max-w-md w-full p-6" onClick={(e) => e.stopPropagation()}>
        <div className="flex justify-between items-start mb-4">
          <div>
            ...
            <p className="text-sm text-cream/65 mt-1">Escanea para usar Tap to Pay</p>
          </div>
          <button onClick={onClose} className="text-cream/45 hover:text-gray-600">
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="mt-6 space-y-3">
          <div className="bg-resona/10 border-l-4 border-blue-500 p-3 rounded">
            <p className="text-sm text-resona-light"><strong>📱 Paso 1:</strong> Escanea el QR con tu móvil</p>
          </div>
          <div className="bg-emerald-500/10 border-l-4 border-green-500 p-3 rounded">
            <p className="text-sm text-emerald-300"><strong>🔓 Paso 2:</strong> Inicia sesión si pide</p>
          </div>
          <div className="bg-ink-800/5 border-l-4 border-purple-500 p-3 rounded">
            <p className="text-sm text-cream/80"><strong>💳 Paso 3:</strong> Tap en "Cobrar con Tap to Pay"</p>
          </div>
          <div className="bg-orange-50 border-l-4 border-orange-500 p-3 rounded">
            <p className="text-sm text-amber-300"><strong>👋 Paso 4:</strong> Cliente acerca su tarjeta → ✅ Cobrado</p>
          </div>
        </div>
        <div className="mt-6 flex gap-3">
          <button
            onClick={() => window.open(paymentLinkQR, '_blank')}
            className="flex-1 px-4 py-2 bg-resona text-white rounded-lg hover:bg-resona-dark transition text-sm font-medium"
          >
            Abrir en Nueva Pestaña
          </button>
          <button
            onClick={() => { navigator.clipboard.writeText(paymentLinkQR); toast.success('Link copiado al portapapeles'); }}
            className="flex-1 px-4 py-2 border border-cream/15 rounded-lg hover:bg-white/5 transition text-sm font-medium"
          >
            Copiar Link
          </button>
        </div>
        ...
        <button onClick={onClose} className="w-full mt-4 px-4 py-3 bg-ink-700 text-cream/75 rounded-lg hover:bg-gray-200 transition font-medium">
          Cerrar
        </button>
      </div>
    </div>
